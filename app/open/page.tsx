import { FLAG_LABELS, isOpen, searchQuestions, STATUS_LABELS, UNRESOLVED } from "@/lib/atlas";

type Params = { q?: string; status?: string; domain?: string; flag?: string; cursor?: string };

const STATUS_NOTES: Record<string, string> = {
  Validated: "人類審査を通過した問いです。どの問いにも、白黒のつく解決基準と出典があります。",
  Screened: "形式・安全性・重複の確認を通過し、人類審査を待っている問いです。論文でいうプレプリントにあたります。",
  Resolved: "解決基準を満たし、証拠が記録された問いです。",
...
export default async function OpenIndex({ searchParams }: { searchParams: Params }) {
  const { q = "", domain, flag, cursor } = searchParams;
  const status = searchParams.status && STATUS_LABELS[searchParams.status] ? searchParams.status : "Validated";
  ...
  const [page, ...everything] = await Promise.all([
    searchQuestions({ query: q, status, domain, flags, cursor, limit: 50 }),
    ...Object.keys(STATUS_LABELS).map((s) => searchQuestions({ query: "", status: s, limit: 50 })),
  ]);
  const domains = [...new Set(everything.flatMap((p) => p?.results.flatMap((r) => r.domain) ?? []))].sort();
  const results = page?.results ?? [];

  const href = (change: Partial<Params>) => {
    const merged: Params = { q: q || undefined, status, domain, flag, ...change };
    const entries = Object.entries(merged).filter((e): e is [string, string] => !!e[1] && !(e[0] === "status" && e[1] === "Validated"));
  ...
  return (
    <>
      ...
      <nav aria-label="状態" className="mt-10 flex flex-wrap gap-x-6 gap-y-2 border-b border-ax-rule text-[14px]">
      ...
      <form action="/open" className="mt-6 flex flex-wrap items-end gap-3 text-[14px]">
        {status !== "Validated" && <input type="hidden" name="status" value={status} />}
        <label className="flex flex-col gap-1.5 text-[12px] text-ax-muted">
          キーワード
          <input name="q" type="search" defaultValue={q} className={`w-48 ${field}`} />
        </label>
        <label className="flex flex-col gap-1.5 text-[12px] text-ax-muted">
          分野
        ...
        {(q || domain || flag) && (
          <a href={href({ q: undefined, domain: undefined, flag: undefined, cursor: undefined })} className="py-2 text-ax-muted underline underline-offset-4 hover:text-ax-ink">
            条件をクリア
          </a>
        )}
      </form>
      ...
      <ul className="mt-3 overflow-hidden rounded-2xl border border-ax-rule bg-white">
        {results.map((r) => (
          <li key={r.id} className="border-t border-ax-rule px-5 py-6 first:border-t-0 sm:px-8">
            ...
            <a href={`/open/q/${r.slug}`} className={`text-[17px] font-bold leading-[1.9] hover:opacity-70 ${isOpen(r.status) ? UNRESOLVED : ""}`}>
              {r.title_i18n?.ja ?? r.title}
            </a>
            <p className="mt-2 flex flex-wrap gap-x-3 gap-y-1 font-jbmono text-[12px] text-ax-muted">
              {r.domain.map((d) => (
